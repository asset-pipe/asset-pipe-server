...
module.exports = class OptimisticBundler {
    constructor({ bundleInProcess, ...options }) {
        const { bundleFeeds } = initUtils({ bundleInProcess });
        this.bundleFeeds = bundleFeeds;
    ...
    async bundle(tag, tags, hashes, type) {
        ...
        const content = await this.bundleFeeds(feeds, type, {
            ...this.options,
            ...this.overrides,
        });
        const hash = hashArray(hashes);
    ...
    async saveFallbackBundle(tag, hash, type, feed) {
        ...
        if (await this.hasBundle(hash, type)) {
            ...
            const bundle = await this.bundleFeeds([feed], type);

...
module.exports = class Router extends EventEmitter {
    ...
    async fetchFeeds(ids) {
        try {
            return await Promise.all(ids.map(file => this.sink.get(file)));
        } catch (err) {
    ...
    async uploadBundle(fileName, bundle) {
        try {
    ...
    async bundle({ type, feedIds, uri }) {
        ...
        const fetchedFeeds = await this.fetchFeeds(feedIds);
        const parsedFeeds = this.parseFeeds(fetchedFeeds);

        const bundle = await this.bundleFeeds(parsedFeeds, type);

        const fileName = `${createHashFromContent(bundle)}.${type}`;
        await this.uploadBundle(fileName, bundle);

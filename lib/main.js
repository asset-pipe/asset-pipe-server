...
module.exports = class Router extends EventEmitter {
    constructor(sink, { bundleInProcess, ...options } = {}) {
        ...
        const {
            ...
            endWorkers,
            booleanWithDefault,
        } = initUtils({ bundleInProcess });

        this.parseBody = parseBody;
        this.validateFeeds = validateFeeds;
        this.bundleAndUpload = bundleAndUpload;
        this.endWorkers = endWorkers;
    ...
    postBundleHandler() {
        return async (req, res, next) => {
            ...
            try {
                ...
                const feedIds = this.validateFeeds(payload);

                this.emit(
                    'info',
                    `Producing and saving asset bundle for requested feeds`
                );
                const response = await this.bundleAndUpload({
                    sink: this.sink,
                    feedIds,
